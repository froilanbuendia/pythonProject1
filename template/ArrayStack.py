...
class ArrayStack(Stack):
    ...
    def add(self, i: int, x: np.object):
        ...
        '''
        if i < 0 or i > self.n: raise IndexError()    #PRECONDITION
        if self.n >= len(self.a): self.resize()
        j = self.n
        while j > i:     #how to format in for loop
            self.a[j] = self.a[j-1]    #shift right
            j -= 1
        self.a[i] = x
        self.n += 1
    ...
    def remove(self, i: int) -> np.object:
        ...
        j = i
        while j < self.n - 1:
            self.a[j: self.n - 2] = self.a[j + 1: self.n - 1]   #shift elements to the left
            j += 1
        self.n -= 1
